...
- `verify_stances.py` — second-pass verification (stance/entity/state/political). Prompts + triggers come from `second_pass` in the config; enum outputs are config-constrained.
- `attachment_utils.py` — download/extract attachment text (PyPDF2, docx; caches to `.extracted.txt`). Image OCR uses OpenAI vision via LiteLLM (opt-in `--use-gemini`, a legacy flag name).
...
- **`second_pass:`** — `model`, `max_workers`, per-field triggers (`stance`, `entity_type`, `state`, `political_affiliation`), and required `prompts.stance` / `prompts.entity` (+ optional `prompts.state` / `.political`).
